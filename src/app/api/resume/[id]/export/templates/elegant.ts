...
function buildElegantSectionContent(section: Section): string {
  ...
  const GOLD = '#d4af37';
  if (section.type === 'summary') return buildSummaryHtml((c as SummaryContent).text || '', 'text-center text-sm leading-relaxed text-zinc-600 italic');
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold" style="color:#2c2c2c">${esc(it.degree)}${it.field ? `, ${esc(it.field)}` : ''}</span>${it.institution ? `<span class="text-sm text-zinc-500"> — ${esc(it.institution)}</span>` : ''}</div><span class="shrink-0 text-xs italic text-zinc-400">${esc(it.startDate)} – ${esc(it.endDate)}</span></div>
