...
function buildRoseSectionContent(section: Section): string {
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => `<div class="rounded-xl border p-4" style="border-color:${ROSE_100}">
      <div class="flex items-baseline justify-between"><h3 class="text-sm font-semibold" style="color:${PRIMARY}">${esc(it.degree)}${it.field ? `, ${esc(it.field)}` : ''}</h3><span class="shrink-0 text-xs" style="color:${ACCENT}">${esc(it.startDate)} &ndash; ${esc(it.endDate)}</span></div>
