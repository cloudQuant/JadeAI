...
function buildLuxeSectionContent(section: Section): string {
  ...
  if (section.type === 'education') {
    return `<div class="space-y-4">${((c as EducationContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:${GOLD}">
      <div class="flex items-baseline justify-between"><h3 class="text-sm font-bold" style="color:${TEXT}">${esc(it.degree)}${it.field ? `, ${esc(it.field)}` : ''}</h3><span class="shrink-0 text-xs italic" style="color:#a8a29e">${esc(it.startDate)} &ndash; ${esc(it.endDate)}</span></div>
